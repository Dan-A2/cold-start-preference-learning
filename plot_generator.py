...
while True:
    ...
    plt.ylabel('F1 Score')
    plt.title(F'Low-Data Regime Performance on {path} Dataset (Averaged over 40 Runs)')
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel('Number of Training Samples')
    plt.ylabel('F1 Score')
    plt.title(F'Extended Data Regime Performance on {path} Dataset (up to 10,000 Pairs)')
    plt.legend()
    plt.grid(True)
